 ...
 */
package tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm;
...
import tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.Trace;
...
public class VmTraceGenerator {
  ...
  public static Trace generateTrace(final TransactionTrace transactionTrace) {
    final tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.VmTrace rootVmTrace = new tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.VmTrace();
    final Deque<tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.VmTrace> parentTraces = new ArrayDeque<>();
  ...
  private static void reduceGasUsed(final tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.Op op) {
    final tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.VmTrace sub = op.getSub();
    if (sub == null) {
      return;
    }
    op.getEx().setUsed(op.getEx().getUsed() - tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.Op.totalGasCost(sub.getOps().stream()));
  ...
  private static void addFrame(
      ...
      final Deque<tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.VmTrace> parentTraces) {
    if ("STOP".equals(traceFrame.getOpcode())) {
      return;
    }

    // boolean addOpToTrace = true;
    tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.VmTrace newSubTrace;
    tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.VmTrace currentTrace = parentTraces.getLast();
    ...
    final Optional<TraceFrame> maybeNextFrame =
        transactionTrace.getTraceFrames().size() > nextFrameIndex
            ...
            : Optional.empty();
    final tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.Op op = new tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.Op();
    // set gas cost and program counter
    op.setCost(traceFrame.getGasCost().orElse(Gas.ZERO).toLong());
    op.setPc(traceFrame.getPc());

    final tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.Ex ex = new tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.Ex();
    // set gas remaining
    ex.setUsed(
    ...
    if (traceFrame.isMemoryWritten() && !isPreviousFrameReturnOpCode) {
      maybeNextFrame
          ...
          .ifPresent(
              memory -> {
                if (memory.length > 0) {
                  ex.setMem(new tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.Mem(BytesValues.trimTrailingZeros(memory[0]).getHexString(), 0));
                }
              });
    }
    ...
    if ("CALL".equals(traceFrame.getOpcode())) {
      newSubTrace = new tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.VmTrace();
   ...
   */
  private static void handleSstore(final TraceFrame traceFrame, final tech.pegasys.pantheon.ethereum.api.jsonrpc.internal.results.tracing.vm.Ex ex) {
    ex.setStore(
        traceFrame
            .getStack()
